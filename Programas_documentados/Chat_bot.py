...
class ChatBotBrain:
    """
    Cerebro del Asistente (LÓGICA CORE):
    Maneja el estado (memoria del usuario), normalización (limpieza de texto) 
    y el motor de intenciones basado en puntuación.
    """
    def __init__(self):
        ...
        # Banco de conocimientos estático (Diccionario de preguntas/respuestas)
        self.faq_profe = {
        ...
        # --- SISTEMA DE EMPATÍA (Frustración) ---
        ...
        # --- MOTOR DE INTENCIONES (Diccionario anidado) ---
        # Clasifica intenciones mediante palabras clave para calcular puntuaciones.
        self.intents = {
            "saludo": {
    ...
    def normalize(self, text: str) -> str:
        ...
        para que el motor compare el texto limpiamente sin fallar por una tilde.
        """
        text = text.lower()
        text = "".join(
    ...
    def evaluate_math(self, text: str) -> str | None:
        """
        CÁLCULOS (Regex): Intenta detectar y resolver sumas, restas, 
        multiplicaciones o divisiones extraídas de texto natural.
        """
    ...
    def detect_name(self, raw_text: str, norm_text: str) -> str | None:
        """
        EXTRACCIÓN DE ENTIDADES (Regex): Detecta si el usuario está diciendo su 
        nombre para guardarlo, o preguntando por él (recuperándolo de la sesión).
        """
        patterns = [
    ...
    def detect_frustration(self, norm_text: str) -> str | None:
        """Detector básico de sentimientos de frustración y desánimo."""
    ...
    def get_response(self, raw_input: str) -> str:
        """
        RUTEO PRINCIPAL: Evalúa las intenciones en orden de prioridad 
        y devuelve la mejor respuesta posible.
        """
        normalized_input = self.normalize(raw_input)
        if not normalized_input:
            return "Parece que no escribiste nada. ¿En qué te puedo ayudar?"

        # Control del 'Modo Profe'
        ...
        # Búsqueda directa si el modo profe está activo
        ...
        # Caso especial para generar fecha y hora dinámicamente
...
class ChatBotApp:
    """
    INTERFAZ GRÁFICA (GUI): Dibuja la ventana y controla los eventos visuales.
    """
    def __init__(self, root: tk.Tk):
        ...
        self.chat_history = []  # Arreglo para exportar al cerrar la app
    ...
    def build_ui(self):
        """Construye todos los elementos visuales (Labels, Text y Entry) de Tkinter."""
        # Header superior
        ...
        # Configuración de etiquetas ("Tags") de estilo para pintar texto
        ...
        # Deshabilitar edición directa del historial para el usuario
    ...
    def protocol_setup(self):
        """Captura el cierre de la ventana ('X') para ejecutar la función de guardar historial."""
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def display_message(self, sender: str, message: str, color_tag: str):
        """
        Inserta un mensaje en el área de historial de forma segura:
        Habilita el widget, inserta el texto coloreado, deshabilita y hace auto-scroll.
        """
        ...
        # Registrar en la lista de historial para exportación final
        self.chat_history.append(f"[{now_str}] {sender}: {message}")

    def send_message(self):
        """
        Captura el texto enviado, lo muestra inmediatamente y 
        programa la respuesta del bot para que haya una pausa natural.
        """
        ...
        # .after() simula retraso natural sin congelar la ventana de Tkinter
        self.root.after(450, lambda: self.bot_reply(raw_text))

    def bot_reply(self, user_text: str):
        """Manda el texto al 'cerebro' y dibuja la respuesta devuelta."""
        response = self.brain.get_response(user_text)
        self.display_message("Bot", response, COLOR_BOT)

    def on_closing(self):
        """EXPORTACIÓN: Genera el archivo .txt con el historial de chat antes de salir."""
